...
export function ImageProcessor() {
  ...
  const handleCropChange = (id: string, crop: { x: number; y: number; width: number; height: number } | null) => {
    const imageIndex = images.findIndex((img) => img.id === id)
    if (imageIndex === -1) return
      ...
      <CardContent className="p-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="upload" className="mt-0">
            <ImageUploader onImagesUploaded={handleImagesUploaded} isProcessing={isProcessing} />
          </TabsContent>
          ...
          <TabsContent value="preview" className="mt-0">
            {images.length > 0 && (
              <div className="space-y-6">
                <ImagePreview images={images} selectedImageId={selectedImageId} onSelectImage={setSelectedImageId} />
